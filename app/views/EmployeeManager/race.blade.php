                ...
                <div class='col-sm-12'>
                  <div class='box bordered-box orange-border' style='margin-bottom:0;'>
                     ...
                     <div class="row box box-transparent">
                        ...
                        <a href="{{ URL::to('employee/config/add-race')}}">
                        <div class="header">
                        <div class="icon-comments"></div>
                        </div>
                        <div class="content">Add Race</div>
                        </a>
                        </div>
                        </div>
                     </div>
                      ...
                      <div class='responsive-table'>
                        <div class='scrollable-area'>
                          <table class='data-table-column-filter table table-bordered table-striped' style='margin-bottom:0;'>
                            ...
                            <tbody>
                             @if(!empty($races)) 
                             @foreach($races as $view) 
                                <tr>
                                    <td><a href="{{ URL::to('employee/config/edit-race/'.$view->id)}}">{{ ucfirst($view->race) }}</a></td>
                                   <td>
                                     <a class="btn btn-primary btn-xs has-tooltip" data-original-title="Edit" data-placement="top" href="{{ URL::to('employee/config/edit-race/'.$view->id)}}">Edit</a>
...
<script>
    $(document).ready(function(){
        var url = "<?php echo URL::to('employee/config/delete-race/'); ?>";
        $('.confirm-delete').click(function(){
        var id = $(this).data('id');
        if(confirm('Are you sure delete this race?')) {
