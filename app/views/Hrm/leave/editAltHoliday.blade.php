              ...
              <div class='row'>
                <div class='col-sm-12'>
                  <div class='page-header'>
                    <h1 class='pull-left'>
                      <i class='icon-ok'></i>
                      <span>Edit Alternate Holiday</span>
                    </h1>
                    <div class='pull-right'>
                      <ul class='breadcrumb'>
                        ...
                        <li class='separator'><i class='icon-angle-right'></i></li>
                        <li class='active'>Edit Alternate Holiday</li>
                      </ul>
                    </div>
                  </div>
                </div>
                ...
                <div class="row">
                  <div class='col-sm-12'>
                      ...
                      <div class='box-content'>
                        <form class='form form-horizontal validate-form' action="{{ URL::to('leave/edit-alt-holiday/'.$id) }}" name="frmaltholiday" id="frmaltholiday" method="post" style='margin-bottom: 0;'>
                                 ...
                                 <select  id='fkHolyId' name='fkHolyId' class="form-control" >
                                     @if(!empty($holiday_list))
                                       @foreach($holiday_list as $k => $htype)
                                          <option value="{{ $htype->id }}" {{ $lview->fkHolyId == $htype->id ? 'selected' : ''}}>{{ $htype->holiday_name.' / '.date('d-m-Y',strtotime($htype->holiday_date)).' - '.date('l',strtotime($htype->holiday_date)) }}</option>
                                       @endforeach
                                     @endif
                                 </select>    
                             </div>
                          ...
                          <div class='form-group'>
                             ...
                             <div class='col-sm-6 controls'>
                               <input class="form-control" data-rule-required='true' data-rule-minlength="2"  id='holiday_name' name='holiday_name' placeholder='Holiday Name' type='text' value="{{ $lview->holiday_name }}">
                             </div>
                          </div>
                          <div class='form-group'>
                             <label class='control-label col-sm-3' for='holiday_date'>Holiday Date</label>
                             <div class='col-sm-6 controls input-group'>
                               <input class='form-control datepicker' data-rule-required='true' id='holiday_date' name='holiday_date' data-format="DD-MM-YYYY"  placeholder='DD-MM-YYYY' type='text' value="{{  $lview->holiday_date!= 0 ? date('d-m-Y',strtotime($lview->holiday_date)) : '' }}">
