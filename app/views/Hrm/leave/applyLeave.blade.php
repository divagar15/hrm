...
@section('content')
  <div class='row' id='content-wrapper'>
            <div class='col-xs-12'>
              ...
              <div class="col-sm-12">
                        <div class="pull-right">
                        <a class="btn btn-warning btn-xs has-tooltip" data-placement="top" data-original-title="Back" href="{{ URL::To('leave/applied') }}">Back</a>
                        </div>
                        </div>
              </div>
                <div class="row">
                  <div class='col-sm-12'>
                      ...
                      <div class='box-content'>
                        <form class='form form-horizontal' id="apply-leave" action="{{ URL::to('leave/apply-leave') }}" name="frmleave" id="frmleave" method="post" style='margin-bottom: 0;'>
                            ...
                            <div class="form-group">
                                    <label class='control-label col-sm-3' for='from_date'>From Date</label>
                                     <div class='controls col-sm-6 input-group'>
                                          <input class='form-control datepicker' autocomplete='off' id='from_date' name='from_date'  data-rule-required="true" placeholder='DD-MM-YYYY' type='text' /> 
                            ...
                            <div class="form-group">
                                    <label class='control-label col-sm-3' for='to_date'>To Date</label>
                                     <div class='controls col-sm-6 input-group'>
                                          <input class='form-control datepicker' autocomplete='off' id='to_date' name='to_date'  data-rule-required="true" placeholder='DD-MM-YYYY' type='text' /> 
                                          <span class="input-group-addon">
                                           <span class="icon-calendar" data-time-icon="icon-time" data-date-icon="icon-calendar"></span>
                                          </span>
                                     </div>
...
<script type="text/javascript">
    ...
    $('#add-row-btn').live('click',function(){
        ...
        if(parseInt(cVal) < parseInt(len)) {
                ...
                vhtml  += '</select>';
                vhtml  += '</div>';
                //<a class="btn-primary btn-xs btn" type="button" id="delete-row-btn-'+crow+'" onclick="deleteRow('+crow+');">Delete Row</a> 
                vhtml  += '</div>';
                vhtml  += '<div class="form-group leave_row_'+crow+'">';
       ...
       $('.datepicker').datepicker({
        ...
        $("#apply-leave").validate({
            submitHandler: function (form) {
              ...
              $(".leave_type").each(function(){
                if($.isNumeric($(this).val())) {
                  console.log(parseInt($(this).val())+"--"+split1[1]+"/"+split2[1]);
                  if((parseInt($(this).val())==7) && (split1[1]!=split2[1])) {
                    i++;
                  }
                }
              });
              if(i!=0) {
                alert("If one of the leave type is No Paid Leave then the month should not cross over in from and to date");
                return false;
              } else {
                form.submit();
              }       
