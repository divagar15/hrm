                ...
                <div class="col-sm-12">
                    <div class="page-header">
                        <h1 class="pull-left">
                            ...
                            <span>Leave Applied Listing</span>
                        </h1>
                        <div class="pull-right">
                            <ul class="breadcrumb">
                                ...
                                <li class="active">Leave Applied Listing</li>
                            </ul>
                        </div>
                    </div>
                </div>
                  ...
                  <div class='box bordered-box orange-border' style='margin-bottom:0;'>
                     <!-- alert box section -->
                      ...
                      <div class="row box box-transparent">
                        ...
                        <a href="{{ URL::to('leave/apply-leave')}}">
                        <div class="header">
                    ...
                    <div class='box'>
                      ...
                      <div class='box-content'>
                        <form class='form form-horizontal validate-form' action="{{URL::to('leave/applied')}}" name="frmleave" id="frmleave" method="post" style='margin-bottom: 0;'>
                                  ...
                                  <div class="row">
                                    <div class="col-sm-6 col-sm-offset-5">
                                      <div class="btn-toolbar">
                                        ...
                                        <a class="btn-default btn resetbtn" href="{{ URL::to('leave/applied'); }}" type="button">Cancel</a>
                                   </div>
                                    </div>
                                  </div>
                           </div>
                    ...
                    <div class='row' style="margin-left:0px;">
			                <div class='col-sm-12'>
			                  <div class='box bordered-box blue-border' style='margin-bottom:0; width: 100%;'>
			                    <div class='box-header blue-background'>
			                      <div class='title'>Leave Applied Listing</div>
			                    </div>
...
<!--                                                         <td><a class="view-leave has-popover" data-toggle='modal' data-empid= "{{ $alist->emp_id }}" data-from="{{ date('d-m-Y',strtotime($alist->from_date)) }}" data-to="{{ date('d-m-Y',strtotime($alist->to_date)) }}" data-off="{{ $alist->off_day == 1 ? 'No' : 'Yes' }}" data-ph="{{ $alist->ph_day == 1 ? 'No' : 'Yes' }}" data-appid="{{ $alist->apply_id }}" href="#leave_list_info">{{ ucwords($alist->emp_name) }}</a></td>
 -->			                                <td>{{ date('d-m-Y',strtotime($alist->from_date)) }}</td>
			                                ...
			                                <td>{{ substr($alist->reason,0,100).'...' }}</td>
                                                        <td>
                                                        @if($alist->status == 1) {{ 'Approved' }}
                                                        @elseif($alist->status == 2) {{ 'Rejected' }}  
                                                        @elseif($alist->status == 3) {{ 'Pending' }} 
                                                        @elseif($alist->status == 4) {{ 'Cancelled' }} 
                                                        @endif
                                                        </td>
                                                        <td>{{  date('d-m-Y',strtotime($alist->postedDate)) }}</td>

                                                      <td><a class="view-leave has-popover btn btn-primary btn-xs" data-toggle='modal' data-empid= "{{ $alist->emp_id }}" data-from="{{ date('d-m-Y',strtotime($alist->from_date)) }}" data-to="{{ date('d-m-Y',strtotime($alist->to_date)) }}" data-off="{{ $alist->off_day == 1 ? 'No' : 'Yes' }}" data-ph="{{ $alist->ph_day == 1 ? 'No' : 'Yes' }}" data-appid="{{ $alist->apply_id }}" href="#leave_list_info"> View </a></td>
